...
if __name__ == "__main__":
    ...
    for nS in nS_list:
        res_folder = f"{res_folder_all}/nS={nS}"
        try:
            os.mkdir(res_folder)
        ...
        print("*****************************************************************************")
        print(f"***Starting simulations for nS={nS}***", flush=True)
        print("*****************************************************************************")

        crm_pool = generate_crmodel(1000, nR, range(1), rho_gamma=rho_gamma, e_mean=e_mean)[0]  # same crm pool for each (rho_gamma, e_mean)

        # f_list for different nS. finer sampling for small nS, coarser for large nS
        f_list = np.concatenate([np.arange(8), np.arange(8, 20, 2), np.arange(20, nS, 4)]) / nS
        f_list = f_list[f_list <= 1]
        if f_list[-1] < 1:
            f_list = np.concatenate([f_list, [1.0]])
